...
1. **XOR-decision matrix** — each evaluation is expressed as an XOR number.
2. **XOR-normalization** — evaluations are normalized element-wise, preserving the XOR structure.
3. **Weighting** — criteria weights are applied via scalar multiplication over XOR numbers.
4. **XOR-aggregation** — an additive operator produces a global XOR-performance score for each alternative.
